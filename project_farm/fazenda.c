...
Fazenda *cadastrarFazenda(Fazenda *fazendas){
	Fazenda *new = (Fazenda *) malloc(sizeof(Fazenda)), *aux = fazendas;

	new->id_fazenda = rand() % 100000;
	printf("\nInsira novamente seu ID de criador: "); scanf("%d", &new->id_criador);
	printf("Insira o nome da fazenda: "); scanf("%s", new->nome);
	printf("Insira a cidade: "); scanf("%s", new->localizacao.cidade);
	printf("Insira o estado: "); scanf("%s", new->localizacao.estado);
	printf("Insira o logradouro: "); scanf("%s", new->localizacao.logradouro);
...
void cadastrarFazendaRebanho(Fazenda *fazenda){ // lincagem
	fazenda->rebanho = cadastrarAnimal(fazenda->rebanho);
}

void mostrarFazendaAnimais(Fazenda *fazenda){ // lincagem
...
int quantSexoAnimais(Fazenda *fazenda, char sexo){
	return countAnimais(fazenda->rebanho, sexo);
}
